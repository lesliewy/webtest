...
sftp -oPasswordAuthentication=no $remoteuser@$remoteip:${remotedatadir}/* ${datadir_tmp} >${logfile} 2>&1    #  15 上不能使用 &>>  sh上不能使用&>   
sftpreturn=$?
...
chkflag1=`find ${datadir_tmp} -name "*.chk"`                             # bsh 中必须加 "
...
# 将chk文件和dat文件转移.
cp $datawithchknoimport ${localchkdatadir} >>${logfile}  2>&1
echo ${datawithchk} |tr ' ' '\n'|sed -e 's/^/-put /g' > ${sftpbatchfile1}

# 将有chk文件的dat文件sftp至remote
sftp -oPasswordAuthentication=no -b ${sftpbatchfile1} $remoteuser@$remoteip:${remotechkdatadir}  > ${logfile1} 2>&1     #  可以达到 &< 效果.
sftpreturn=$?
if [ "$sftpreturn" -ne 0 ]; then
...
sftp -oPasswordAuthentication=no -b ${sftpbatchfile2} $remoteuser@$remoteip:${remotedatadir}  > ${logfile2} 2>&1
sftpreturn=$?
if [ "$sftpreturn" -ne 0 ]; then
